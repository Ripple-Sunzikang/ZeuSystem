...
## 分阶段实施与文档化（当任务复杂时必须遵循）

- **分阶段实现**：若某个子任务规模较大或涉及多名开发者，应将实现工作分为若干明确的阶段（例如：词法/语法 → 语义/IR → 代码生成 → 汇编/链接/验证），并在每个阶段完成后提交成果。
- **阶段总结文档**：每个阶段完成后必须在 `docs/` 中新增阶段总结文档（推荐命名：`docs/STAGE_<n>_<short-name>.md`），内容包括：目标、完成项、剩余问题、接口说明、测试结果、下一步计划。
- **使用阶段文档继续开发**：后续阶段的开发人员应以阶段总结文档为权威说明，任何设计变更需在文档中记录并通过版本控制提交。
- **不可跳过或简化要求**：不得以“实现简化版”或“部分实现”为借口跳过关键功能。若项目太大无法一次完成，必须采用分阶段实施并编写阶段总结文档以保证可追溯性和可交付性。

该节为强制性要求，所有贡献者在开始大型任务前必须阅读并遵守。